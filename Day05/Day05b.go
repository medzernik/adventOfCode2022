...
func RunDay05b() {
	...
	for scanner.Scan() {
		...
		stacks[ti] = append(stacks[ti], stacks[fi][l-amount:l]...) //adds amount of items from "from" stack to "to" stack
		stacks[fi] = stacks[fi][:l-amount]                         //removes amount of items from "from" stack
